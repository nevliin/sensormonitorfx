 ...
 */
public class SensorChart extends LineChart implements DataChangeListener {
    ...
    public void dataChanged(String graphname) {
        if (graphname != null) {
            ...
            // Remove points outside the display area of the LineChart
            for (int i = 0; i < points.size(); i++) {
                if (points.get(i).x < chartdata.getXMin()) {
                    points.remove(i);
                } else {
                    break;
                }
            }
            // If the graph is visible, add the points to the corresponding XYChart.Series
            if (chartdata.get(graphname).isVisible()) {
    ...
    public void visibilityChanged(String graphname, boolean isVisible) {
        setGraphVisible(graphname, isVisible);
    }

    @Override
    public void axisChanged() {
        if (chartdata.getXMin() != ((NumberAxis) this.getXAxis()).getLowerBound()
                || chartdata.getXMax() != ((NumberAxis) this.getXAxis()).getUpperBound()
                || chartdata.getYMin() != ((NumberAxis) this.getYAxis()).getLowerBound()
                || chartdata.getYMax() != ((NumberAxis) this.getYAxis()).getUpperBound()) {
            updateAxis();
        }
    }
